...
function c3vCTM_honeycomb(T_flipped::TensorMap{A, S, 2, 1}; symmetrize = false) where {A, S}
    ...
    if symmetrize
        ...
        if norm(T_unflipped - rotl120_pf_honeycomb(T_unflipped)) > 1.0e-10
            @warn "Tensor is not C3 symmetric. Error = $(norm(T_unflipped - rotl120_pf_honeycomb(T_unflipped)))"
        end
    end
...
function calculate_projectors(scheme, trunc)
    @tensor opt = true mat[χout Dout; χin Din] := scheme.L[χout DNW; χN] * scheme.C[χN χNE] * scheme.R[χNE DE; χin] *
        scheme.T[DNW DE DC] * flip(scheme.T, [2 3])[Din DC Dout]
...
function renormalize_corners!(scheme, W)
    return @tensor opt = true scheme.C[χout; χin] := scheme.L[χWc DNW; χN] * scheme.C[χN χNE] * scheme.R[χNE DE; χE] *
        scheme.T[DNW DE DC] * flip(scheme.T, [2 3])[Din Dout DC] *
        W[χE Din; χin] * flip(W, [1 2 3])[χWc Dout; χout]
        # W[χE Din; χin] * conj(W[χWc Dout; χout])
end

function renormalize_edges!(scheme, W)
    @tensor opt = true scheme.L[χout Dout; χin] := scheme.L[χ1 D1; χ2] * scheme.T[D1 D2 D3] * flip(scheme.T, [1 2 3])[Dout D4 D3] *
        W[χ2 D2; χin] * flip(W, [1 2 3])[χ1 D4; χout]
        # W[χ2 D2; χin] * conj(W[χ1 D4; χout])
    return @tensor opt = true scheme.R[χout Dout; χin] := flip(scheme.R, 2)[χ1 D1; χ2] * flip(scheme.T, [1 2 3])[D2 D3 D1] * flip(scheme.T, 3)[D2 D4 Dout] *
        W[χ2 D4; χin] * flip(W, [1 2 3])[χ1 D3; χout]
...
function _contract_site_large(scheme::c3vCTM_honeycomb)
    return @tensor opt = true scheme.C[χSWR; χNW] * scheme.C[χNW; χNL] * scheme.L[χNL DNW; χNR] *
        ...
        scheme.T[DNW DE DC] * flip(scheme.T, [3])[DSE DW DC]
end
