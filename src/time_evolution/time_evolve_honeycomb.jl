function evolution_operator_honeycomb(ce_alg::ClusterExpansion, β::Number; T_conv = ComplexF64, canoc_alg::Union{Nothing, Canonicalization} = nothing)
    if β == 0.0
        @error "TBA"
    ...
    lattice = ClusterExpansions.Honeycomb()
