...
function solve_cluster(lattice, T, cluster, PEPO, β, twosite_op, onesite_op, spaces; nn_term = nothing, symmetry = nothing, verbosity = 2, solving_loops = true, svd = true)
    ...
    elseif length(sites_to_update) ∈ [1, 2]
        ...
        levels_to_update, solutions = solve_index(lattice, T, A, RHS, conjugated, sites_to_update, levels_to_update, get_direction(lattice, dir), cluster.N, spaces; verbosity, svd)
    else
...
function get_nontrivial_terms(lattice::Triangular, N; prev_clusters = [[(0, 0)]])
    ...
    for cluster_indices in prev_clusters # iterate over all previous clusters
        ...
        for (k₁, j₁) in cluster_indices
            for (k₂, j₂) in [(k₁ + 1, j₁), (k₁ - 1, j₁), (k₁, j₁ + 1), (k₁, j₁ - 1), (k₁ + 1, j₁ - 1), (k₁ - 1, j₁ + 1)]
