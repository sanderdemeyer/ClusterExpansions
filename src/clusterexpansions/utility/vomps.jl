...
function VOPEPO(ctm_alg, envspace::ElementarySpace, truncspace::ElementarySpace, miniter::Int, maxiter::T;
    check_fidelity::Bool = false, verbosity::Int = 0, ctm_tol::Float64 = 1e-7) where {T <: Function}
    return VOPEPO(ctm_alg, envspace, truncspace, miniter, maxiter, check_fidelity, verbosity, ctm_tol)
end

function VOPEPO(ctm_alg, envspace::ElementarySpace, truncspace::ElementarySpace, miniter::Int, maxiter::Int = 2;
     check_fidelity::Bool = false, verbosity::Int = 0, ctm_tol::Float64 = 1e-7)
    return VOPEPO(ctm_alg, envspace, truncspace, miniter, i -> maxiter, check_fidelity, verbosity, ctm_tol)
end
...
function get_vomps_A(B, env₀, trunc_alg::VOPEPO)
...
function get_vomps_b(A::Tuple{T,T}, B, env₀, trunc_alg::VOPEPO) where {E,S,T <: AbstractTensorMap{E,S,2,4}}
    A₁, A₂ = A
...
function approximate_state(
    ...
    for i = 1:trunc_alg.maxiter(iter)
        ...
        # Bnew, info = lssolve(apply_A, vomps_b, LSMR(verbosity = trunc_alg.verbosity + 1, maxiter = 1000, tol = 1e-10))
        println(info)
        println("Gradient is $(norm(apply_A(B, Val(false)) - vomps_b))")
        Bcheck = apply_A(Bnew, Val(false))
        println("norm diff is $(norm(Bcheck-vomps_b)) / $(norm(vomps_b)) (and norm of Bcheck = $(norm(Bcheck)))")
        if trunc_alg.check_fidelity
            ϵ = fidelity_vomps(A, Bnew, envspace_fidelity, ctm_alg_fidelity)
            push!(ϵs, ϵ)
            println("Fidelity is $ϵ")
        end
        B = (1-α)*B + α*Bnew
        B /= norm(B)
        push!(Bs, B)
