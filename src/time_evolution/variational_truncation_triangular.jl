...
function TTNTruncation(trscheme::TruncationScheme, ctm_alg::PEPSKit.CTMRGAlgorithm, envspace::ElementarySpace, n::Int;
    tol::Float64 = 1e-10, k::Int = 2, maxiter::Int = 100, check_fidelity::Bool = false, verbosity::Int = 0)
    TTNTruncation(trscheme, ctm_alg, envspace, n, tol, k, maxiter, check_fidelity, verbosity)
end
...
function check_convergence(Ws, Ews)
    ...
    return sum([abs(Zs[i] - Zs[1]) for i = 2:length(Zs)]) / length(Zs)
    # println("Zs = ", Zs)
